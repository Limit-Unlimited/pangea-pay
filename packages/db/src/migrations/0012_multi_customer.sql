-- Migration 0012: multi-customer support
-- Adds web_user_customer_links junction table and active_customer_id on web_users.
-- Backfills from existing web_users.customer_id so existing users are unaffected.

CREATE TABLE `web_user_customer_links` (
...
-- Backfill junction table from existing primary customer links
INSERT INTO `web_user_customer_links` (`id`, `user_id`, `customer_id`, `role`, `is_primary`, `status`)
SELECT UUID(), `id`, `customer_id`, 'owner', true, 'active'
FROM `web_users`
WHERE `customer_id` IS NOT NULL;
--> statement-breakpoint

-- Set active_customer_id to match the current customer_id for all existing users
